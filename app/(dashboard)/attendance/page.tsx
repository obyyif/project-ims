...
type AttendanceRecord = {
  ...
  status: string;
  date: string;
  schedule?: { subject?: { name?: string } };
};

// ── Teacher: attendance summary per schedule ──
type ScheduleItem = {
  id: string;
  day: string;
  subject: { name: string };
...
export default function AttendancePage() {
    ...
    const fetchData = async () => {
      try {
        if (role === "student") {
          ...
          // Teachers: load schedule list to pick from
  ...
  // ── Teacher: load attendance sheet when schedule changes ──
  useEffect(() => {
    if (role !== "teacher" || !selectedScheduleId) return;
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Lihat rekap absensi per jadwal mengajar</p>
        </div>

        {/* Schedule selector */}
        <div className="rounded-3xl bg-white p-6 card-float space-y-4">
          <h3 className="text-sm font-bold text-slate-700">Pilih Jadwal</h3>
          {isLoading ? (
            <p className="text-sm text-slate-400">Memuat jadwal...</p>
          ) : schedules.length === 0 ? (
            <p className="text-sm text-slate-400 italic">Belum ada jadwal mengajar.</p>
          ) : (
            <div className="grid gap-2 sm:grid-cols-2">
              {schedules.map((s) => (
                ...
                >
                  <p className="text-sm font-bold text-slate-900">{s.subject?.name}</p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {s.classroom?.name} • {s.day} {s.start_time?.substring(0, 5)}–{s.end_time?.substring(0, 5)}
                  </p>
                </button>
              ))}
            </div>
          ...
          <div className="rounded-3xl bg-white p-6 card-float">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-bold text-slate-700">
                Rekap: {selected?.subject?.name} — {selected?.classroom?.name}
              </h3>
              {loadingSheet && <span className="text-xs text-slate-400 animate-pulse">Memuat...</span>}
            </div>
        ...
        <div className="rounded-3xl bg-gradient-to-br from-indigo-600 to-violet-600 p-5 text-white">
          <p className="text-[10px] font-bold uppercase tracking-wider text-white/70 mb-2">Kebijakan SMK</p>
          <p className="text-sm leading-relaxed text-white/90">Minimal kehadiran 90% untuk syarat kelulusan semester.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-3xl bg-white p-6 card-float">
          <div className="flex items-center justify-between mb-5">
            ...
            <p className="font-bold text-slate-700">{monthNames[currentMonth]} {currentYear}</p>
        ...
        <div className="space-y-5">
          <div className="rounded-3xl bg-white p-6 card-float">
            <h3 className="text-sm font-bold uppercase tracking-[0.15em] text-slate-400 mb-4">Ringkasan Bulanan</h3>
          ...
          <div className="rounded-3xl bg-white p-6 card-float">
            <h3 className="text-sm font-bold uppercase tracking-[0.15em] text-slate-400 mb-4">Aktivitas Terakhir</h3>
            <div className="space-y-3">
              {activityLog.length === 0 ? (
                <p className="text-sm text-slate-400 italic text-center py-4">Belum ada riwayat kehadiran.</p>
              ) : (
                activityLog.map((act, i) => (
                  <div key={i} className="flex items-center gap-3 rounded-2xl bg-slate-50 border border-slate-100 p-3">
                    <div className={`h-9 w-9 rounded-xl ${act.color} flex items-center justify-center text-white text-xs font-bold shrink-0`}>→</div>
